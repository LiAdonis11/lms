        ...
        <section className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            ...
            {activeBorrowings.length > 0 && (
              <span className="badge badge-default">{activeBorrowings.length}</span>
            )}
          </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {activeBorrowings.map((borrowing, index) => (
                <div key={borrowing.id} className={`card p-6 animate-fade-in stagger-${(index % 5) + 1}`}>
                  ...
                  <h3 className="text-lg font-semibold mb-1 line-clamp-1">{borrowing.title || 'Book'}</h3>
                  <p className="text-sm text-zinc-500 mb-4">{borrowing.author || 'Unknown Author'}</p>
                  
                  <div className="space-y-2 mb-4 text-sm">
                    <div className="flex justify-between">
                      <span className="text-zinc-400">User ID</span>
                      <span className="text-zinc-600">#{borrowing.user_id}</span>
                    </div>
                    <div className="flex justify-between">
                  ...
                  >
                    {updating === borrowing.id ? 'Processing...' : 'Mark as Returned'}
                  </button>
                </div>
              ))}
            </div>
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            {returnedBorrowings.length > 0 && (
              <span className="badge badge-default">{returnedBorrowings.length}</span>
            )}
          </div>
